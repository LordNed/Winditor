...
namespace BrawlLib.Modeling.Triangle_Converter
{
    ...
    public class PrimitiveBrawl
    {
        public PrimitiveBrawl(PrimType type)
        {
            Type = type;
    ...
    {
        private List<PrimitiveBrawl> m_PrimitivesVector;
        ...
        public TriStripper(uint[] TriIndices, Weight[] weights = null) //, ushort[] NodeIds, int[] ImpTable)
        {
            ...
            m_StripID = 0;
            m_FirstRun = true;
            m_PrimitivesVector = new List<PrimitiveBrawl>();
	    ...
	    public List<PrimitiveBrawl> Strip()
        {
	        if (!m_FirstRun) 
            {
